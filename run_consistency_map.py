...
def main():
    ...
    parser.add_argument(
        "input", type=Path, help="Path to the input directory of multi-view data (e.g., result/lpmvs/straight)")
    parser.add_argument("-o", "--output", type=Path,
                        help="Output directory (default: result/consistency/<input>)")
    ...
    multiviewdata = utils.read_multiview(
        args.input, read_images=False, verbose=False)
    ...
    for view_i in progress_bar:
        ...
        reference_view = utils.read_multiview(
            args.input, view_select=[view_i], verbose=False)[0]
        neighbor_indices = multiviewdata.get_neighbor_index_vector(
            view_i, args.num_neighbors)

        width, height = reference_view.size()
        imgs_position_diff = np.empty(
            (args.num_neighbors, height, width), dtype=np.float32)
        imgs_angle_diff = np.empty(
            (args.num_neighbors, height, width), dtype=np.float32)
        for i, neighbor_view_i in enumerate(neighbor_indices):
            # Read a neighbor view
            neighbor_views = utils.read_multiview(
                args.input, view_select=[neighbor_view_i], verbose=False)[0]

            # Compute difference between reference view and neighbor view in terms of position and angle
            img_position_diff, img_angle_diff = strandtools.eval_consisntency(
                reference_view, neighbor_views)
        ...
        img_position_diff_average = np.nansum(
            imgs_position_diff**2 * imgs_weight, axis=0) / np.nansum(imgs_weight, axis=0)

        # Mask out pixels that have small number of valid neighbors because they are not reliable
        img_position_diff_average[img_valid_sum <
                                  args.num_least_consisten_neigbor] = np.nan
        ...
        img_depth_consistency = np.exp(
            -(img_position_diff_average) / (2 * args.sigma**2))
        ...
        img_mask_new = np.bitwise_and(np.logical_not(
            img_mask_nan), reference_view.img_mask > 0)
        reference_view.img_mask = img_mask_new.astype(np.uint8) * 255
        points, _ = reference_view.getDirectionalPoint()
        colors = np.clip(
            img_depth_consistency[img_mask_new] * 255, 0, 255).astype(np.uint8)
        ...
        if args.write_ply:
            utils.write_ply(
                output_sub / configs.filename_pointcloud, points, colors)
        cv2.imwrite(str(output_sub / configs.filename_consistency),
                    img_depth_consistency.astype(np.float32))
